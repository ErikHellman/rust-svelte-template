...
async fn list_notes(
    ...
    Ok(Json(repo::list(&state.db, &user.id).await?))
}
...
async fn create_note(
    ...
    let note = repo::create(&state.db, &user.id, &input.title, &input.body).await?;
...
async fn update_note(
    ...
    repo::update(&state.db, &user.id, &id, &input.title, &input.body)
        .await?
        .map(Json)
        .ok_or(AppError::NotFound)
}
